...
<config xmlns="http://artifactory.jfrog.org/xsd/2.1.6" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.jfrog.org/xsd/artifactory-v2_1_6.xsd">
    ...
    <addons>
        ...
        <showAddonsInfoCookie>1546658246764</showAddonsInfoCookie>
    </addons>
    <security>
        ...
        {{- if .Values.artifactory.ldap.enabled }}
        <ldapSettings>
            <ldapSetting>
                <key>{{ .Values.artifactory.config.ldapSetting.key }}</key>
                <enabled>{{ .Values.artifactory.config.ldapSetting.enabled }}</enabled>
                <ldapUrl>{{ .Values.artifactory.config.ldapSetting.ldapUrl }}</ldapUrl>
                <search>
                    <searchFilter>{{ .Values.artifactory.config.ldapSetting.searchFilter }}</searchFilter>
                    <searchBase>{{ .Values.artifactory.config.ldapSetting.searchBase }}</searchBase>
                    <searchSubTree>true</searchSubTree>
                    <managerDn>{{ .Values.artifactory.config.ldapSetting.managerDn }}</managerDn>
                    <managerPassword>{{ .Values.artifactory.config.ldapSetting.managerPassword }}</managerPassword>
                </search>
                <autoCreateUser>{{ .Values.artifactory.config.ldapSetting.autoCreateUser }}</autoCreateUser>
        ...
        <ldapGroupSettings>
            <ldapGroupSetting>
                <name>{{ .Values.artifactory.config.ldapGroupSettings.name }}</name>
                <groupBaseDn>{{ .Values.artifactory.config.ldapGroupSettings.groupBaseDn }}</groupBaseDn>
                <groupNameAttribute>cn</groupNameAttribute>
                <groupMemberAttribute>memberOf</groupMemberAttribute>
                <subTree>true</subTree>
                <filter>{{ .Values.artifactory.config.ldapGroupSettings.filter }}</filter>
                <descriptionAttribute>description</descriptionAttribute>
                <strategy>DYNAMIC</strategy>
                <enabledLdap>{{ .Values.artifactory.config.ldapSetting.key }}</enabledLdap>
            </ldapGroupSetting>
        </ldapGroupSettings>
        {{- else }}
        <ldapSettings/>
    ...
    {{- if .Values.artifactory.config.proxy.enabled }}
    <proxies>
        <proxy>
            <key>{{ .Values.artifactory.config.proxy.key }}</key>
            <host>{{ .Values.artifactory.config.proxy.host }}</host>
            <port>{{ .Values.artifactory.config.proxy.port }}</port>
    ...
    <proxies/>
    {{- end }}
    {{- if .Values.artifactory.config.reverseProxy.enabled }}
    <reverseProxies>
        <reverseProxy>
            <key>nginx</key>
            ...
            <serverName>{{ .Values.artifactory.config.reverseProxy.serverName }}</serverName>
            <serverNameExpression>*.{{ .Values.artifactory.config.reverseProxy.serverName }}</serverNameExpression>
            <sslCertificate>xxx</sslCertificate>
            <sslKey>xxx</sslKey>
            <dockerReverseProxyMethod>subDomain</dockerReverseProxyMethod>
            <useHttps>true</useHttps>
            <useHttp>false</useHttp>
            <sslPort>443</sslPort>
            <httpPort>80</httpPort>
            <reverseProxyRepositories/>
            <artifactoryServerName>artifacts-nc.{{ .Values.artifactory.config.reverseProxy.serverName }}</artifactoryServerName>
