...
def ref = 'refs/changes/06/550806/13'
...
if(Jenkins.instance.getAllItems(hudson.model.Job).size() == 0) {
    ...
    if (jobsList) {
        ...
        try {
            ...
            // try to seed jobs. If seed script is not approved approve and rerun.
            if(createdJob.scheduleBuild2(0, new ParametersAction(params)).get(15, TimeUnit.MINUTES).getResult() != hudson.model.Result.SUCCESS) {
                // approve super seed script
                def scriptApproval = org.jenkinsci.plugins.scriptsecurity.scripts.ScriptApproval.get()
                scriptApproval.getPendingScripts().each { scriptApproval.approveScript(it.hash) }

                // schedule seeding and wait for the job to complete (15 min timeout)
