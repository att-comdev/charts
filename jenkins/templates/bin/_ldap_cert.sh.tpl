...
if [[ "$copy_cert" = "true" ]]; then
  ...
  wget {{.Values.conf.config.jenkins.ldap.cacert_url}} -O $JENKINS_HOME/certificate/SBC-Ent-Root-CA.cer --user {{.Values.conf.config.jenkins.ldap.artif_username}} --password {{.Values.conf.config.jenkins.ldap.artif_password}} || true
